...
from parser2 import UPSParser
from parser2 import WebRequestParser
from parser2 import AResponseParser
from parser2 import select_timeout_from_ACommands
from parser2 import select_timeout_from_ACommunicate
import time
import datetime
import json
from parser2 import buy_to_pckid
#from parser2 import generate_ToPack
from parser2 import generate_Commands

UPShost = '127.0.0.1'
Webhost = '127.0.0.2'
#worldhost = 'vcm-7990.vm.duke.edu'
#worldhost = 'vcm-5869.vm.duke.edu'
...
def handle_world_request(request, worldsocket, UPSsocket):
    ...
    print("sending ACommands to world")
    print(acommand)
    send_request(worldsocket, acommand)
    # update status whether
    ap.update_status()
    acommunicate = ap.generate_ALoadingFinished()
    print("sending ACommunicate to UPS")
    print(acommunicate)
...
def send_CMDs(worldsocket, UPSsocket):
    while True:
        time.sleep(5)
        acommand, acommu = generate_Commands()
        if acommand and acommu:
            print("sending APack to world")
            print(acommand)
            send_request(worldsocket, acommand)
            print("sending AOrderPlaced to UPS")
            print(acommu)
            send_request(UPSsocket, acommu)
...
class WebProxy:
    ...
    def run(self):
        ...
        sendToPack = threading.Thread(target=send_CMDs, args=(self.worldsocket, self.UPSsocket))
        sendToPack.start()
